...
@testset "Drag" begin
    # Dead Starlink satellite
    jd0, _ = datevec2jdate([2026.0, 4, 1, 4, 35, 6.711])
    jdf, _ = datevec2jdate([2026.0, 4, 20, 15, 27, 52.904])

    state0 = build_cartesian_state(
        [-5606.729752163668, 4117.957550663997, 0.0463621986960463, 0.6027848756963391, 0.8011474204182883, 7.497559187017876],
        jd0,
        :J2000,
    )
    output = build_base_output(-1.0, [:sma], :J2000, jdf)
    options = build_numerical_options(
        use_non_spherical = true,
        use_third_body_sun = true,
        use_third_body_moon = true,
    ...
    input = build_numerical_input(state0, options, output)
    out = propagate(input).sma[1]

    # Make sure the results match
    @test isapprox(6938.085187930652, out, atol = 4.0)
end
